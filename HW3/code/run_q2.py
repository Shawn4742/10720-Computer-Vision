...
# parameters in a dictionary, global variables?
params = {}
...
def foward_and_loss(x, y, params):
    h1 = forward(x,params,'layer1')
    probs = forward(h1,params,'output',softmax)
    loss, _ = compute_loss_and_acc(y, probs)
    return loss

# check by one-step computing
# need to start over the training
# initialize a layer
initialize_weights(2,25,params,'layer1')
initialize_weights(25,4,params,'output')
# forward
    ...
    if len(v.shape) > 1:
        for i in range(v.shape[0]):
            for j in range(v.shape[1]):
                v[i, j] += eps
                f1 = foward_and_loss(x,y,params)

                v[i, j] -= 2*eps
                f2 = foward_and_loss(x,y,params)
